...
class tasks extends panel
{
    ...
    public function todays_tasks_due()
    {
        ...
        $tasks = $this->db->get_where('user_tasks', array('user_tasks.assigned_user_id' => $this->user->umId, 'user_tasks.status !=' => 'Completed', 'user_tasks.perc_of_completion <' => 100))->result();
   ...
   public function tasks_assigned_by_me()
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            try {
                ...
                if ($task_check->num_rows() > 0) {
                    ...
                    $task->task_comments = $this->db->get_where('task_comments', array('task_comments.task_id' => $this->input->post('task_id')))->result();
	...
	$this->db->select('tasks.task_id, tasks.priority,user_tasks.group_ref, user_tasks.created_user_name assigned_by, DATE_FORMAT(user_tasks.task_comment_date, ("%d-%m-%Y %h:%i %p"))as date, 
        ...
        user_tasks.perc_of_completion,tasks.created_by,user_tasks.assigned_user_id ,tasks.due_date,user_tasks.approved_status, usermaster.umusername as approved_by, um.umusername as rejected_by, task_comments.comment as comment,
        ...
        //$task_report = $this->db->get_where('tasks',"DATE_FORMAT(user_tasks.task_comment_date,('%Y-%m-%d')) = '" . $this->input->post('comment_date') . "'".$search)->result();
